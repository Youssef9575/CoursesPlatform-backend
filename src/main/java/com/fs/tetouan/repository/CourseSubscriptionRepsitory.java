...
public interface CourseSubscriptionRepsitory extends JpaRepository<CourseSubscription,Integer> {
	...
	@Query("SELECT i.training FROM CourseSubscription i WHERE i.user.id = :userId")
	...
	@Query("SELECT i FROM CourseSubscription i WHERE i.user.id = :idUser and  i.training.id = :idTrain")
	CourseSubscription getCourseSubscription(long idTrain, long idUser) ;
}
